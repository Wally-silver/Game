...
export class GameMainController extends Component {
  ...
  protected update(dt: number): void {
    if (this.battleRoot?.active) {
      try { App.getServices().battleManager.update(dt); } catch (error) { console.warn('[GameMainController] battle update failed', error); }
    }
  }

  private showHome(): void {
    this.clearFlow();
    App.getServices().battleManager.stopBattle();
  ...
  private showBattle(): void {
    ...
    this.popup.open(candidates, (ruleId) => {
      ...
      if (!ok) {
        this.hud?.appendEventLog('【错误】规则选择失败，请重试。');
        return;
      }
      this.hud?.appendEventLog('【系统】镇规生效，战局开始。');
      this.hud?.refreshFromRuntimeState(app.battleManager.getRuntimeState());
    });
