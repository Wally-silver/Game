import { Button, Canvas, Color, Graphics, Label, Layers, Layout, Node, UITransform, Vec3, Widget, director, view } from 'cc';
...
export class SceneUIFactory {
  ...
  public static ensureSafePanel(parent: Node, name: string, options?: { width?: number; height?: number }): Node {
    ...
    this.ensureSolidBg(panel, new Color(22, 28, 38, 220));
    return panel;
  }
  ...
  public static ensureVerticalGroup(parent: Node, name: string, spacingY = 12): Node {
    ...
    if (trans.width <= 0 || trans.height <= 0) trans.setContentSize(640, 200);
  ...
  public static ensureButton(parent: Node, name: string, text: string, cb?: () => void): { node: Node; button: Button; label: Label } {
    ...
    this.ensureSolidBg(node, new Color(52, 92, 170, 255));
    const labelNode = this.ensureRootOnce(node, "Label");
    const label = labelNode.getComponent(Label) ?? labelNode.addComponent(Label);
    (labelNode.getComponent(UITransform) ?? labelNode.addComponent(UITransform)).setContentSize(trans.width, trans.height);
    label.string = text;
    label.fontSize = 22;
    label.color = new Color(255, 255, 255, 255);
  ...
  private static ensureSolidBg(node: Node, color: Color): void {
    const gfx = node.getComponent(Graphics) ?? node.addComponent(Graphics);
    const trans = node.getComponent(UITransform) ?? node.addComponent(UITransform);
    gfx.clear();
    gfx.fillColor = color;
    gfx.roundRect(-trans.width / 2, -trans.height / 2, trans.width, trans.height, 10);
    gfx.fill();
  }
}
