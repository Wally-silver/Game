...
export class BattleManager {
  ...
  public update(deltaTime: number): void {
    ...
    this.buildingTickAccumulator += deltaTime; this.eventTickAccumulator += deltaTime;
    if (this.buildingTickAccumulator >= BATTLE_DEFAULTS.buildingTickInterval) { this.buildingTickAccumulator = 0; this.tickBuildingAndResidents(); }
    if (this.eventTickAccumulator >= BATTLE_DEFAULTS.eventTickInterval) { this.eventTickAccumulator = 0; this.tickEvents(); }
  ...
  private endBattle(success: boolean): void {
    this.runtime.running = false; this.runtime.ended = true; this.runtime.success = success;
    this.settlementData = {
      success,
