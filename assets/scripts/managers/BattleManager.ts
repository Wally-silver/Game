...
export class BattleManager {
  public static readonly STABILIZE_COST = 28;
  ...
  public setupNewBattle(): RuleModel[] {
    ...
    this.eventBus.emit(EVENT_NAME.BATTLE_EVENT_TRIGGERED, { id: 'opening_tip', name: '开场提示', desc: '新的一天开始了，请选择今日镇规。', effect_order: 0, effect_joy: 0, effect_gold: 0, effect_goal: 0 });
  ...
  public update(deltaTime: number): void {
    ...
    this.logAccumulator += deltaTime;
    if (dirtyResource || this.runtime.timer !== (this.lastTopState?.timer ?? -1)) {
      this.emitResourceIfChanged();
      if (this.logAccumulator >= 3) {
        this.logAccumulator = 0;
        console.log(`[BattleManager] tick timer=${Math.ceil(this.runtime.timer)} order=${Math.round(this.runtime.order)} joy=${Math.round(this.runtime.joy)} gold=${Math.round(this.runtime.gold)} progress=${Math.round(this.runtime.goalProgress)}`);
      }
    }
    if (dirtyBuilding) {
      this.emitBuildingsIfChanged();
  ...
  public toggleBuildingOvertime(buildingId: string): { ok: boolean; message: string; buildingName: string } {
    this.buildingSystem.toggleOvertime(buildingId);
    this.runtime.buildings = this.buildingSystem.getRuntimeBuildings();
    this.emitBuildingsIfChanged();
    const b = this.runtime.buildings.find((x) => x.id === buildingId);
    return { ok: true, message: `${b?.name ?? buildingId} 加班状态已切换`, buildingName: b?.name ?? buildingId };
  }

  public toggleBuildingPause(buildingId: string): { ok: boolean; message: string; buildingName: string } {
    this.buildingSystem.togglePause(buildingId);
    this.runtime.buildings = this.buildingSystem.getRuntimeBuildings();
    this.emitBuildingsIfChanged();
    const b = this.runtime.buildings.find((x) => x.id === buildingId);
    return { ok: true, message: `${b?.name ?? buildingId} 暂停状态已切换`, buildingName: b?.name ?? buildingId };
  }
  ...
  public stabilizeTown(): { ok: boolean; message: string } {
    ...
    if (this.runtime.gold < BattleManager.STABILIZE_COST) return { ok: false, message: '金币不足，无法组织安抚。' };
    this.runtime.gold -= BattleManager.STABILIZE_COST;
  ...
  private checkEndConditions(): void {
    if (this.runtime.order <= 0 || this.runtime.joy <= 0) return this.endBattle(false);
    if (this.runtime.goalProgress >= BATTLE_DEFAULTS.targetProgress) return this.endBattle(true);
    if (this.runtime.timer > 0) return;
    const success = this.runtime.order > 0 && this.runtime.joy > 0 && (this.runtime.goalProgress >= BATTLE_DEFAULTS.targetProgress || this.runtime.gold >= BATTLE_DEFAULTS.targetGold);
